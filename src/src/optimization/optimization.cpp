...
//从keyframes.txt中读取关键帧，其中每一个关键帧包含一个由IKF得到的激光里程计位姿，以及该激光里程计关键帧时刻对应的由差值得到的RTK位姿
//从yaml文件中读取优化参数、激光雷达里程计(同IMU)与RTK之间的外参
bool Optimization::Init(int stage) {
    stage_ = stage;

    if (!LoadKeyFrames("./../data/mapdata/keyframes.txt", keyframes_)) {
    ...
    lidar_continuous_num_ = yaml["optimization"]["lidar_continuous_num"].as<int>();
...
void Optimization::Run() {
    ...
    SaveG2O("./../data/mapdata/before.g2o");
    ...
    SaveG2O("./../data/mapdata/after.g2o");
...
void Optimization::AddRTKEdges() {
    ...
    for (auto& kfp : keyframes_) {
        ...
        if (kf->rtk_heading_valid_) {
            ...
            auto rk = new g2o::RobustKernelCauchy();//rtk_outlier_th_是一个预先设定的阈值，用于确定观测值与模型预测值之间的差异超过多少时被认为是异常值。
            rk->setDelta(rtk_outlier_th_);
            edge->setRobustKernel(rk);
            optimizer_.addEdge(edge);
...
void Optimization::SaveResults() {
    ...
    // 写入文件
    system("rm ./../data/mapdata/keyframes.txt");
    std::ofstream fout_lio_tumpose("./../data/evo/lio_keframes.txt",std::ios::trunc);
    std::ofstream fout_lc_tumpose("./../data/evo/loopclosure_keframes.txt",std::ios::trunc);
    std::ofstream fout("./../data/mapdata/keyframes.txt");
    for (auto& kfp : keyframes_) {
        kfp.second->Save(fout);
        kfp.second->Save_lio_tum_pose(fout_lio_tumpose);
        kfp.second->Save_lc_tum_pose(fout_lc_tumpose);
    }
    fout.close();
    fout_lio_tumpose.close();
    fout_lc_tumpose.close();
}
//通过Umeyama进行位置拉齐，将lidar里程计的初始位置对齐到RTK的初始位置
//算法原理：https://zhuanlan.zhihu.com/p/532444005
...
void Optimization::LoadLoopCandidates() {
    std::ifstream fin("./../data/mapdata/loops.txt");
    if (!fin) {
        LOG(WARNING) << "cannot load file: ./../data/mapdata/loops.txt";
        return;
    }
