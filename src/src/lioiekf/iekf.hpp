#ifndef XMF_IESKF_HPP
#define XMF_IESKF_HPP
...
class IESKF {
   public:
    ...
    struct Options {
        ...
        double gyro_var_ = 0.1;       // 陀螺测量标准差1e-5
        double acce_var_ = 0.1;       // 加计测量标准差1e-2
        double bias_gyro_var_ = 0.0001;  // 陀螺零偏游走标准差1e-6
        double bias_acce_var_ = 0.0001;  // 加计零偏游走标准差1e-4
...
bool IESKF<S>::UpdateUsingCustomObserve(IESKF::CustomObsFunc obs) {
    ...
    Mat18T Pk, Qk;
    // LOG(INFO)<<options_.num_iterations_;
...
/*原来高翔版本先更新后投影，按照书上3.63的公式来的*/
 ...
 /*self自己改先投影后更新（按照书上8.21的公式写的）两个版本都可以运行
    Mat18T J = Mat18T::Identity();
    Vec3d dtheta = (R_.inverse() * start_R).log();
    J.template block<3, 3>(6, 6) = Mat3T::Identity() - 0.5 * SO3::hat(dtheta);
    cov_ = J * cov_ * J.inverse();
