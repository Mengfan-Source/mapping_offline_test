...
bool LioIEKF::Init(const std::string &config_yaml) {
    if (!LoadFromYAML(config_yaml)) {//获取雷达和IMU的外参
...
void LioIEKF::Align() {
    ...
    pcl::transformPointCloud(*scan_undistort_, *scan_undistort_trans, TIL_.matrix().cast<float>());//转到IMU坐标系下
...
void LioIEKF::Undistort() {
    ...
    if (options_.save_motion_undistortion_pcd_) {
        xmf::SaveCloudToFile("./../data/tempout/before_undist.pcd", *cloud);
    }
    ...
    std::for_each(std::execution::par_unseq, cloud->points.begin(), cloud->points.end(), [&](auto &pt) {
        ...
        Vec3d pi = ToVec3d(pt);
        Vec3d p_compensate = TIL_.inverse() * T_end.inverse() * Ti * TIL_ * pi;//lidar系下的点转到IMU系下，再转到这一个雷达帧末尾时刻的位置，最后再转回Lidar下
    ...
    if (options_.save_motion_undistortion_pcd_) {
        xmf::SaveCloudToFile("./../data/tempout/after_undist.pcd", *cloud);
    }
}
...
void LioIEKF::Predict() {
    ...
    // }
    //对IMU状态先进行平滑然后再预测
    IMUPtr temp_imu = std::make_shared<xmf::IMU>(0, Vec3d::Zero(), Vec3d::Zero());
    for(auto imu_temp  = measures_.imu_.begin();imu_temp<measures_.imu_.end()-1;imu_temp++){
        auto &&head = *(imu_temp);
        auto &&tail = *(imu_temp+1);
        temp_imu->gyro_<<0.5*(head->gyro_(0)+tail->gyro_(0)),0.5*(head->gyro_(1)+tail->gyro_(1)),0.5*(head->gyro_(2)+tail->gyro_(2));
        temp_imu->acce_ <<0.5*(head->acce_(0)+tail->acce_(0)),0.5*(head->acce_(1)+tail->acce_(1)),0.5*(head->acce_(2)+tail->acce_(2));
        temp_imu->timestamp_ = head->timestamp_;
        ieskf_.Predict(*temp_imu);
        imu_states_.emplace_back(ieskf_.GetNominalState());
    }
    ieskf_.Predict(*(measures_.imu_.back()));
    imu_states_.emplace_back(ieskf_.GetNominalState());
}

void LioIEKF::PCLCallBack(const sensor_msgs::PointCloud2::ConstPtr &msg) { sync_->ProcessCloud(msg); }

void LioIEKF::LivoxPCLCallBack(const livox_ros_driver2::CustomMsg::ConstPtr &msg) { sync_->ProcessCloud(msg); }
