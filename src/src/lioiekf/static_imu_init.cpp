#include "lioiekf/static_imu_init.h"
...
// }
/*
1.计算队列中IMU角速度、加速度的均值和方差
2.根据计算出的加速度均值估计重力加速度方向，并计算重力加速度的值
3.考虑估计出的重力加速度值，重新计算加速度的均值和方差
4.检查计算出来的IMU角速度和加速度的方差，如果大于设置的option里面的最大值，认为IMU初始化失败
5.将计算出的角速度和加速度的均值赋值给本类中的init_bg_ 与 init_ba_
*/
