...
void IncNdt3d::ComputeResidualAndJacobians(const SE3& input_pose, Mat18d& HTVH, Vec18d& HTVr) {
    ...
    // LOG(INFO)<<num_residual_per_point;
    std::vector<int> index(source_->points.size());
    for (int i = 0; i < index.size(); ++i) {
        index[i] = i;
    }
    ...
    const double info_ratio = 0.01;  // 每个点反馈的info因子  0.01
...
void IncNdt3d::BuildNDTEdges(xmf::VertexPose* v, std::vector<EdgeNDT*>& edges) {
    assert(grids_.empty() == false);
    SE3 pose = v->estimate();
