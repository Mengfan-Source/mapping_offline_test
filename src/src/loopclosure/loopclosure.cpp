...
void LoopClosure::DetectLoopCandidates() {
    ...
    for (auto iter_first = keyframes_.begin(); iter_first != keyframes_.end(); ++iter_first) {
        ...
        for (auto iter_second = iter_first; iter_second != keyframes_.end(); ++iter_second) {
            ...
            Vec3d dt = kf_first->lidar_pose_.translation() - kf_second->lidar_pose_.translation();
            ...
            if (t2d < range_th) {
                ...
                LoopCandidate c(kf_first->id_, kf_second->id_,kf_first->lidar_pose_.inverse() * kf_second->lidar_pose_);
...
void LoopClosure::ComputeForCandidate(xmf::LoopCandidate& c) {
    ...
    auto build_submap = [this](int given_id, bool build_in_world) -> CloudPtr {
        ...
        for (int idx = -submap_idx_range; idx < submap_idx_range; idx += 4) {
            ...
            pcl::io::loadPCDFile("./../data/mapdata/" + std::to_string(id) + ".pcd", *cloud);
            xmf::RemoveGround(cloud, 0.1);//剔除地面点，这里的地面点不是地面坐标系下的，是lidar坐标系下，直接按照Z轴剔除地面点
            ...
            if (!build_in_world) {
                ...
                Twb = keyframes_.at(given_id)->lidar_pose_.inverse() * Twb;
            }
    ...
    auto submap_kf1 = build_submap(kf1->id_, true);//ture--->将submap转到世界坐标系下

    CloudPtr submap_kf2(new PointCloudType);
    pcl::io::loadPCDFile("./../data/mapdata/" + std::to_string(kf2->id_) + ".pcd", *submap_kf2);
    ...
    c.Tij_ = kf1->lidar_pose_.inverse() * SE3(q, t);
...
void LoopClosure::SaveResults() {
    ...
    std::ofstream fout("./../data/mapdata/loops.txt",std::ios::trunc);
    for (const auto& lc : loop_candiates_) {
