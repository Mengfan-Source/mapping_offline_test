...
void Keyframe::SaveAndUnloadScan(const std::string &path) {
    if (cloud_) {
        xmf::SaveCloudToFile(path + "/" + std::to_string(id_) + ".pcd", *cloud_);
        cloud_ = nullptr;
    }
}
...
void Keyframe::Save_lio_tum_pose(std::ostream &os) {
    auto save_SE3 = [](std::ostream &f, SE3 pose) {
        auto q = pose.so3().unit_quaternion();
        Vec3d t = pose.translation();
        f << t[0] << " " << t[1] << " " << t[2] << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w();
    };
    os<< std::setprecision(18) << timestamp_ << " ";
    save_SE3(os, lidar_pose_);
    os << std::endl;
}
void Keyframe::Save_lc_tum_pose(std::ostream &os) {
    auto save_SE3 = [](std::ostream &f, SE3 pose) {
        auto q = pose.so3().unit_quaternion();
        Vec3d t = pose.translation();
        f << t[0] << " " << t[1] << " " << t[2] << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w();
