...
//一帧IMU数据来了以后，更新IMU的预积分值，更新预积分量(R、V、P)关于零偏(b_a、b_g)的雅可比,更新预积分的协方差矩阵
...
//     LOG(INFO) << "noise cov: " << noise_gyro_acce_;
...
NavStated IMUPreintegration::Predict(const xmf::NavStated &start, const Vec3d &grav) const {
    SO3 Rj = start.R_ * dR_;
